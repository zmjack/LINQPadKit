...
"""
        ...
        public static ITreeGraphNode ParseNode<T>(T root, Func<T, string> textSelector, Func<T, T[]> childrenSelector)
        {
            var children = childrenSelector(root);
            var node = new Node
            {
                Text = textSelector(root),
                Children = children?.Where(x => x is not null).Select(x => ParseNode(x, textSelector, childrenSelector)).ToArray()
            };
            return node;
        }
